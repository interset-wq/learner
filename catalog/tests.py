...
from .models import Author, Genre, Language, Tag, Book, BookInstance, Record


def create_user(username='testuser', password='testpass123', is_staff=False):
    return User.objects.create_user(username, f'{username}@test.com', password, is_staff=is_staff)
...
def create_book(title='Test Book', author_name='Test Author'):
    ...
    return Book.objects.create(title=title, author=author, summary='A test book.', language=language, genre=genre)


def create_book_instance(book=None, status='a'):
    if book is None:
        book = create_book()
    return BookInstance.objects.create(book=book, imprint='Test Editions', status=status)
...
class RecordModelTest(TestCase):
    def test_str(self):
        bi = create_book_instance()
        record = Record.objects.create(book_instance=bi, borrower=create_user(), borrow_date=date.today())
...
class BorrowBookViewTest(TestCase):
    ...
    def test_borrow_book_post(self):
        response = self.client.post(reverse('catalog:borrow-book', args=[self.instance.id]))
        ...
        self.assertTrue(Record.objects.filter(book_instance=self.instance, borrower=self.user).exists())

    def test_borrow_book_requires_login(self):
        self.client.logout()
        response = self.client.get(reverse('catalog:borrow-book', args=[self.instance.id]))
...
class ReturnBookViewTest(TestCase):
    ...
    def test_return_book_post(self):
        response = self.client.post(reverse('catalog:return-book', args=[self.instance.id]))
        self.assertRedirects(response, reverse('catalog:my-borrowed'))
        self.instance.refresh_from_db()
