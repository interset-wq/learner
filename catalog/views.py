...
from catalog.forms import BookForm, BookSearchForm, RenewBookForm
...
def user_profile(request, username):
    ...
    borrowed = BookInstance.objects.filter(borrower=profile_user, status="o").order_by(
        "due_back"
    )
    history = Record.objects.filter(borrower=profile_user).order_by("-borrow_date")[:20]
    context = {
...
class BorrowHistoryListView(LoginRequiredMixin, generic.ListView):
    ...
    def get_queryset(self):
        return Record.objects.filter(borrower=self.request.user).order_by(
            "-borrow_date"
        )
